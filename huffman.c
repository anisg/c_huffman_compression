...
String compress(String b){
	...
	//==== STAT ====
	printf("before:%d, now:%d (%.1f%% of original ; %+d bytes)\n", b.n, size, ((float)size)/((float)b.n)*100.0,((int64_t)size)-((int64_t)b.n));
	printf("----\n");
	printf("header:%d, compressed:%d, total:%d\n", hsize, ((cost+(8-cost%8))/8), size);
	printf("Huffman tree max depth: %d, nb of characters:%d\n",  md, n);
	huffmanDelete(r);
